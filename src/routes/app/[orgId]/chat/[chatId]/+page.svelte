<script lang="ts">
  ...
  async function deleteMessage(messageId: string, isGroup: boolean, typ: NormMsg['typ']) {
    const t = toast.loading("Deleting message", { dismissable: false })
    try {
      // Check if it's a voice message
      const isVoiceMessage = typ === 'voice'
                  ...
                  >
                    ...
                    {:else if message.typ === 'voice'}
                      <VoicePlayer
                        ...
                        on:delete={({ detail }) => deleteMessage(detail.messageId, detail.isGroup, 'voice')}
                      />
                    {/if}
                  ...
                  {#if message.from_id === user.id && message.typ === 'text' && !('isOptimistic' in message && message.isOptimistic)}
                    ...
                    >
                      ...
                      <Button
                        ...
                        on:click={() => deleteMessage(message.id, messages.isGroup, 'text')}
