...
function parseChatSlug(chatSlug: string | undefined): [boolean, string] | undefined {
  let isGroup = false
  switch (chatSlug?.charAt(0)) {
    case '@': break
    case '-': isGroup = true; break
...
export const POST: RequestHandler = async ({ request, locals: { supabase, auth: {user} }, params: { orgId, chatId: chatSlug } }) => {
  if (!user) return json({ error: "Unauthorized" }, {status: 401})
  const chatSlugParsed = parseChatSlug(chatSlug)
  if (!orgId || !chatSlugParsed) return json({ error: "Page not found" }, {status: 404})
  let [isGroup, chatId] = chatSlugParsed
  try {
    const data = addSchema.parse(await request.json())
    const { count: orgCount, error: orgErr } = await supabase.from('organisations_users').select("user_id", { count: 'exact', head: true }).eq('organisation_id', orgId).eq('user_id', user.id)
    if (orgErr) {
    ...
    if (isGroup) {
      const { count: grpCount, error: grpErr } = await supabase.from('chat_group_members').select("user_id", { count: 'exact', head: true }).eq('group_id', chatId).eq('user_id', user.id)
      if (grpErr) {
      ...
      const { data: user, error: usrErr } = await supabase.from('users').select("id").eq('username', chatId).single()
      if (usrErr) {
    ...
    if (data.type === 'text') {
      const id = nanoid()
      const { error } = await (isGroup ? supabaseSR.from("group_messages").insert({
        id,
        by_id: user.id,
        data: data.content,
        group_id: chatId,
      ...
      if (error) {
        captureException(error, { tags: { supabase: isGroup ? 'group_messages' : 'messages' }})
