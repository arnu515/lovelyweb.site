...
export const load: LayoutLoad = async ({ parent, params: { orgId }}) => {
  ...
  const {supabase} = data
  const orgListPromise = supabase.from("organisations").select("id, name, plan")
  ...
  const { data: org, error: orgError } = await supabase.from("organisations").select("*").eq("id", orgId).maybeSingle()
  if (orgError) {
    captureException(orgError, { tags: { supabase: 'organisations' }})
    error(500, { message: orgError.message });
  }
