...
export const load: PageLoad = async ({ parent }) => {
  ...
  const { count, error: orgError } = await data.supabase
    .from('organisations_users')
    .select('*', { count: 'exact', head: true })
    .eq('user_id', data.auth.user.id)
    .limit(1);
  if (orgError) {
  ...
  const invitesPromise = data.supabase
    .from('org_invites')
    .select(
      'message, org_id, invitee, created_at, organisations ( name, description, link, users!organisations_owner_id_fkey ( name, avatar_url ) )'
    )
